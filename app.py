...
for name, symbol in stock_list.items():
    ...
    with col1:
        ...
        st.markdown(
            ...
            f"5日: <span style='color:#2E86C1'>{safe_value(latest['5MA'])}</span>, "
            f"10日: <span style='color:#28B463'>{safe_value(latest['10MA'])}</span>, "
            f"20日: <span style='color:#AF7AC5'>{safe_value(latest['20MA'])}</span>"
            f"</div>",
    ...
    with col2:
        ...
        st.markdown("### 📎 布林通道")
        st.markdown(f"<div style='font-size:18px'>中: {safe_value(latest['BB_MID'])}, 上: {safe_value(latest['BB_UPPER'])}, 下: {safe_value(latest['BB_LOWER'])}</div>", unsafe_allow_html=True)
