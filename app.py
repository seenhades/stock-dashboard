...
# === 1. 設定區 ===
st.set_page_config(layout="wide", page_title="高級股票量化監控系統")
st.title("🚀 全方位量化交易監控系統 (歐美台日全相容版)")
    ...
    with tab:
        ...
        for name, symbol in stocks.items():
            ...
            if data.empty or len(data) < 30:
                st.warning(f"{name} ({symbol}) 無法獲取足夠歷史資料")
                continue
            ...
            if last['Close'] < last['MA20']: buy_score += 0.5
            
            sell_score = 0
            if last['K'] > 70 and last['K'] < last['D']: sell_score += 1
            if last['RSI'] > 70: sell_score += 1
            if last['Close'] > last['UpperBB']: sell_score += 1
            ...
            with c1:
                ...
                st.write(f"**趨勢強度:** {'🔥 強' if last['ADX'] > 25 else '💤 弱'}")
            ...
            with c4:
                st.write("🔍 **技術面快訊**")
                if last['K'] < 20: st.caption("✨ KD 進入超賣區")
                if last['Vol_Ratio'] > 1.5: st.caption("⚡ 爆量異動中")
                if last['RSI'] > 75: st.caption("🚩 RSI 過熱提醒")
                if last['Close'] > last['EMA12']: st.caption("📈 站上短天期生命線")
